...
unsigned char i2c_write(void *context, unsigned char slave_addr, unsigned char reg_addr, unsigned char length, unsigned char const *data)
{
	...
	PAL_I2C_SetByteInterval(hmpu->Init.hI2c, MPU6050_WRITE_DELAY);
	...
	PAL_I2C_SetByteInterval(hmpu->Init.hI2c, 0);
...
ErrorStatus PAL_MPU6050_Init(PalMPU6050_HandleTypeDef *Handle)
{
	...
	mpu_init_param.accel_fsr = Handle->Init.MPU6050_AccelFsr;
	mpu_init_param.gyro_fsr = Handle->Init.MPU6050_GyroFsr;
	
	if(mpu_init(Handle, &mpu_init_param) != 0) 
		return ERROR; // 初始化失败
	
	/* Wake up all sensors. */
	if(mpu_set_sensors(Handle, INV_XYZ_GYRO | INV_XYZ_ACCEL) != 0) 
		return ERROR;
	/* Push both gyro and accel data into the FIFO. */
	if(mpu_configure_fifo(Handle, INV_XYZ_GYRO | INV_XYZ_ACCEL) != 0)
		return ERROR;
	
	if(dmp_load_motion_driver_firmware(Handle) != 0)
		return ERROR;
	
	if(dmp_set_orientation(Handle, inv_orientation_matrix_to_scalar(gyro_orientation)) != 0)
		return ERROR;
	
  if(dmp_enable_feature(Handle, DMP_FEATURE_6X_LP_QUAT|DMP_FEATURE_TAP|	//设置dmp功能
		    DMP_FEATURE_ANDROID_ORIENT|DMP_FEATURE_SEND_RAW_ACCEL|DMP_FEATURE_SEND_CAL_GYRO|
		    DMP_FEATURE_GYRO_CAL) != 0)
		return ERROR;
	
	if(dmp_set_fifo_rate(Handle, Handle->Init.SampleRate) != 0)
		return ERROR;
	
	if(mpu_set_dmp_state(Handle, 1) != 0)
		return ERROR;
	
	Handle->hexti.Init.GPIO.GPIOx = Handle->Init.Int_GPIOPort;
	Handle->hexti.Init.GPIO.GPIO_Pin = Handle->Init.Int_GPIOPin;
	Handle->hexti.Init.GPIO.GPIO_Mode = GPIO_Mode_IN_FLOATING;
	Handle->hexti.Init.Exti.EXTI_Mode = EXTI_Mode_Interrupt;
	Handle->hexti.Init.Exti.EXTI_Trigger = EXTI_Trigger_Rising;
	Handle->hexti.Init.Interrupt.PreemptionPriority = Handle->Init.Int_PremptionPriority;
	Handle->hexti.Init.Interrupt.SubPriority = Handle->Init.Int_SubPriority;
	Handle->hexti.Init.CallbackFn = Handle->Init.Int_Callback;
...
{
	PAL_EXTI_LineCmd(&Handle->hexti, NewState);
}

void PAL_MPU6050_EXTI_IRQHandler(PalMPU6050_HandleTypeDef *Handle)
{
...
ErrorStatus PAL_MPU6050_Update(PalMPU6050_HandleTypeDef *Handle)
{
	long quat[4]; 
	unsigned char more;
	...
	do
	{
		if(dmp_read_fifo(Handle, gyro, accel, quat, &sensor_timestamp, &sensors, &more) != 0)
			return ERROR;
	}
	while(more);
	...
	if(mpu_get_gyro_sens(Handle, &gyro_sense) != 0)
		return ERROR;
	...
	if(mpu_get_accel_sens(Handle, &accel_sense) != 0)
		return ERROR;
