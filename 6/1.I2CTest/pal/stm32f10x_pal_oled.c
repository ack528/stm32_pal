...
static void DrawCharator(PalOled_HandleTypeDef *Handle, uint32_t Unicode)
{
	...
	if(idx < 0) return; // 未找到对应的字形
	
	const PalGlyph_TypeDef glyph = Handle->Font->Glyphs[idx];
	...
	if(Handle->TextRegionWidth!=0 && Handle->TextRegionHeight!=0)
	{
		...
		// 如果下一个字符的宽度超过了文本框
		if(Handle->CursorX+glyph.Dwx0 >= Handle->TextRegionX+Handle->TextRegionWidth)
		{
			Handle->CursorX = Handle->TextRegionX; // 让光标回到起始点
	...
	// 绘制背景
	FillRect(Handle, Handle->CursorX, Handle->CursorY - Handle->Font->FBBYoff - Handle->Font->FBBy, glyph.Dwx0, Handle->Font->FBBy);
	
	// 绘制字形
	DrawBitmapEx(Handle, Handle->CursorX + glyph.BBxoff0x, Handle->CursorY - glyph.BByoff0y - glyph.BBh, glyph.BBw, glyph.BBh, glyph.Bitmap);
